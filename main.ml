...
let rec string_of_expr = function
  ...
  | ConstVector_int (_, v) -> Printf.sprintf "ConstVector_int(%s)" (String.concat ", " (List.map string_of_int v))
  | ConstVector_float (_, v) -> Printf.sprintf "ConstVector_float(%s)" (String.concat ", " (List.map string_of_float v))
  | ConstMatrix_int (_, _, m) -> Printf.sprintf "ConstMatrix_int(%s)" (String.concat ", " (List.map (fun v -> "[" ^ (String.concat ", " (List.map string_of_int v)) ^ "]") m))
  | ConstMatrix_float (_, _, m) -> Printf.sprintf "ConstMatrix_float(%s)" (String.concat ", " (List.map (fun v -> "[" ^ (String.concat ", " (List.map string_of_float v)) ^ "]") m))
  | Var id -> Printf.sprintf "Var(%s)" id
  | BinOp(op, e1, e2) ->
      Printf.sprintf "BinOp(%s, %s, %s)" op (string_of_expr e1) (string_of_expr e2)
  | Neg e -> Printf.sprintf "Neg(%s)" (string_of_expr e)
  | Abs e -> Printf.sprintf "Abs(%s)" (string_of_expr e)
  | Angle(e1, e2) -> Printf.sprintf "Angle(%s, %s)" (string_of_expr e1) (string_of_expr e2)
  | Dimension e -> Printf.sprintf "Dimension(%s)" (string_of_expr e)
  | MatrixMultiply(e1, e2) -> Printf.sprintf "MatrixMultiply(%s, %s)" (string_of_expr e1) (string_of_expr e2)
  | DotProduct(e1, e2) ->
      Printf.sprintf "DotProduct(%s, %s)" (string_of_expr e1) (string_of_expr e2)
  ...
  | IndexAccess(e1, e2) -> Printf.sprintf "IndexAccess(%s, %s)" (string_of_expr e1) (string_of_expr e2)
...
let rec string_of_stmt = function
  ...
  | AssignIndex(e1, e2) -> Printf.sprintf "AssignIndex(%s, %s)" (string_of_expr e1) (string_of_expr e2)
  | If(cond, then_stmt, Some(else_stmt)) ->
    let s1 = List.fold_left (fun acc stmt -> acc ^ (string_of_stmt stmt) ^ "\n") "" then_stmt in
    let s2 = List.fold_left (fun acc stmt -> acc ^ (string_of_stmt stmt) ^ "\n") "" else_stmt in
      Printf.sprintf "If(%s, %s, %s)"
        (string_of_expr cond) s1 s2
  | If(cond, then_stmt, None) ->
    let s1 = List.fold_left (fun acc stmt -> acc ^ (string_of_stmt stmt) ^ "\n") "" then_stmt in
      Printf.sprintf "If(%s, %s, None)" (string_of_expr cond) s1
  | While(cond, body) ->
    let s = List.fold_left (fun acc stmt -> acc ^ (string_of_stmt stmt) ^ "\n") "" body in
  ...
  | For(id, start, end_, body) ->
    let s = List.fold_left (fun acc stmt -> acc ^ (string_of_stmt stmt) ^ "\n") "" body in
      Printf.sprintf "For(%s, %s, %s, %s)" (string_of_stmt id) (string_of_expr start) (string_of_expr end_) s
