...
let rec type_check_stmt env stmt =
    ...
    | AssignIndex(index_expr, value_expr) ->
        ...
        in
        let base_var = check_base_is_var index_expr in
        (* Check the type of the base variable and value *)
        let base_type = (try List.assoc base_var env with Not_found -> failwith ("Undefined variable: " ^ base_var)) in
        let value_type = type_of_expr env value_expr in
        (match base_type with
        | Vector_int _ -> if value_type = Int then env else failwith "Type mismatch in vector index assignment"
        | Vector_float _ -> if value_type = Float then env else failwith "Type mismatch in vector index assignment"
        | Matrix_int(_, _) -> if value_type = Int then env else failwith "Type mismatch in matrix index assignment"
        | Matrix_float(_, _) -> if value_type = Float then env else failwith "Type mismatch in matrix index assignment"
        | _ -> failwith "Index assignment must target a vector or matrix")
    | If(e, s1, s2_opt) ->
        let t = type_of_expr env e in
        if t = Bool then
