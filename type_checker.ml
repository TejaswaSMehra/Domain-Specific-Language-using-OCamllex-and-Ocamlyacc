...
let rec type_of_expr env expr =
    ...
    | BinOp (op, e1, e2) -> (
        ...
        | "-", Vector_int n, Vector_int m when n = m -> Vector_int n
        | "-", Vector_float n, Vector_float m when n = m -> Vector_float n
        | "+", Matrix_int (a, b), Matrix_int (c, d) when a = c && b = d ->
            Matrix_int (a, b)
        | "+", Matrix_float (a, b), Matrix_float (c, d) when a = c && b = d ->
            Matrix_float (a, b)
        | "-", Matrix_int (a, b), Matrix_int (c, d) when a = c && b = d ->
            Matrix_int (a, b)
        | "-", Matrix_float (a, b), Matrix_float (c, d) when a = c && b = d ->
    ...
    | Dimension e -> (
        ...
        | Vector_float _ -> Int
        | Matrix_int (_, _) -> Int
        | Matrix_float (_, _) -> Int
        | _ -> failwith "Type error in dimension" )
    | MatrixMultiply (e1, e2) -> (
        match (type_of_expr env e1, type_of_expr env e2) with
        | Matrix_float (a, b), Matrix_float (c, d) when b = c ->
            Matrix_float (a, d)
    ...
    | Transpose e -> (
        match type_of_expr env e with
        | Matrix_int (a, b) ->
            Matrix_int (b, a) (* Transpose swaps dimensions *)
        | Matrix_float (a, b) ->
            Matrix_float (b, a) (* Transpose swaps dimensions *)
...
let rec type_check_stmt env stmt =
    ...
    | AssignIndex (index_expr, value_expr) ->
        ...
        if base_type = value_type then env
        else failwith "Type error in index assignment"
    | If (e, s1, s2_opt) ->
        let t = type_of_expr env e in
        if t = Bool then
    ...
    | While (e, s) ->
        let t = type_of_expr env e in
        if t = Bool then List.fold_left type_check_stmt env s
        else failwith "Type error in while loop"
    | For (v, e1, e2, s) -> (
        match v with
        | Assign (_, e3) ->
            ...
            if t1 = Int && t2 = Int && t3 = Int then
              List.fold_left type_check_stmt env1 s
            else failwith "Type error in for loop"
        | _ -> failwith "For loop statement should be an assignment" )
    | Print id ->
        if List.mem_assoc id env then env
        else failwith "Undefined variable in print statement"

let type_check_program program =
    match program with stmts -> List.fold_left type_check_stmt [] stmts
